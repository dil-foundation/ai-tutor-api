...
CRITICAL_OPINION_FILE = os.path.join(os.path.dirname(__file__), '..', 'data', 'stage6_exercise3.json')
...
def get_topic_by_id(topic_id: int):
    ...
    try:
        with open(CRITICAL_OPINION_FILE, 'r', encoding='utf-8') as f:
            topics = json.load(f)
...
async def get_all_topics(current_user: Dict[str, Any] = Depends(require_student)):
    ...
    try:
        print(f"📁 [API] Reading topic file from: {CRITICAL_OPINION_FILE}")
        with open(CRITICAL_OPINION_FILE, 'r', encoding='utf-8') as f:
            topics = json.load(f)
...
async def get_topic(topic_id: int, current_user: Dict[str, Any] = Depends(require_student)):
    ...
    try:
        ...
        return {
            ...
            "controversy_level": topic_data['controversy_level'],
...
async def critical_opinion_builder(topic_id: int, current_user: Dict[str, Any] = Depends(require_student)):
